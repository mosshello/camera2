...
### ⚠️ WYSIWYG 拍照必须使用 AVCaptureVideoDataOutput（2026-04-27）
`AVCapturePhotoOutput` 输出原生分辨率（前置 1080×1420，后置 1920×1440），与屏幕比例不同，合成时 crop 不对称导致画面变形或错位。
正确方案：用两个 `AVCaptureVideoDataOutput` 实时接收与预览同步的帧到 `_latestFrontFrame` / `_latestBackFrame`，快门按下时直接从 buffer 取帧合成。
注意：VideoDataOutput 需要在 `begin/commitConfiguration` 块外添加（它们不影响 session 配置）。
